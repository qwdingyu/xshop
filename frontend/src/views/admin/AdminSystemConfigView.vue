        ...
        <section class="config-section danger-section" aria-labelledby="data-maintenance-title">
          <div class="config-section-head">
            <div>
              ...
              <p>清除商品、卡密、订单、营销、余额、邮件日志、请求日志、旧管理员审计、限流和幂等状态；保留系统配置、支付配置、分类、API Key 和迁移记录，并写回本次清理凭证。</p>
            </div>
          </div>
          <div class="danger-panel">
            <label>
              ...
              <input
                ...
                type="text"
                autocomplete="off"
                :placeholder="CLEAR_BUSINESS_DATA_CONFIRMATION"
              />
            </label>
            <button
              type="button"
            ...
            >
              {{ clearingBusinessData ? '清除中…' : '清除业务数据' }}
            </button>
          </div>
...
const canClearBusinessData = computed(() =>
  !clearingBusinessData.value
    && clearBusinessConfirmText.value.trim() === CLEAR_BUSINESS_DATA_CONFIRMATION,
)
...
async function clearBusinessData() {
  ...
  const confirmed = await askConfirm('确认清除所有业务数据？此操作不可恢复，系统配置、支付配置、分类、API Key 和迁移记录会保留，旧管理员审计会被本次清理凭证替代。')
  if (!confirmed) return

  clearingBusinessData.value = true
  clearBusinessDataStatus.value = ''
  try {
    const result = await clearAdminBusinessData(token.value, CLEAR_BUSINESS_DATA_CONFIRMATION)
    clearBusinessConfirmText.value = ''
    clearBusinessDataStatus.value = `已清除 ${result.deleted} 条数据，保留审计凭证 ${result.retainedAuditId}。`
    showToast('业务数据已清除', 'success')
  } catch (err: any) {
